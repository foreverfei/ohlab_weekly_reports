这段主程序来自b站上的一个up主，我按照视频的讲解进行了调试和运行
```python
import argparse
import warnings
import os
...
def parse_args():
    ...
    parser.add_argument('--datadir', type=str, default=r'C:\Users\coaxh\Desktop\mysecondtraining', help='这个路径下需要有poses_bounds.npy以及images文件夹')
    # 网络模型参数
    ...
    parser.add_argument('--iters', type=int, default=100, help='训练一共迭代多少次')
    ...
    # 数据集参数
    parser.add_argument("--factor", type=int, default=8, help='数据处理时的下采样倍率')
    parser.add_argument("--no_ndc", type=bool, default=True, help='不使用归一化设备坐标系，环绕场景设为True，前向场景一般用ndc，但也可以不用，效果会差一些')
    parser.add_argument("--lindisp", type=bool, default=True, help='是否基于视差图采样而不是深度')
    parser.add_argument("--spherify", type=bool, default=True, help='用于360度场景')
    ...
    parser.add_argument("--i_valid", type=int, default=200, help='验证的频率，会保存权重，计算验证集loss和psnr')
    parser.add_argument("--i_test", type=int, default=100, help='测试的频率，会生成新视角图像，比较耗时，建议隔久一些')
